    ...
    while (parsing && !error) {
        ...
        if (token.type == YAML_KEY_TOKEN) {
            ...
            if (token.type == YAML_SCALAR_TOKEN) {
                if (!found_socket && strcmp((char *)token.data.scalar.value, "socket") == 0) {
                    found_socket = true;
                }
                else if (found_socket && strcmp((char *)token.data.scalar.value, "port") == 0) {
                    yaml_token_delete(&token);
                    ...
                    if (token.type == YAML_SCALAR_TOKEN) {
                        ...
                        parsing = false;  // Port found, exit the loop
                    }
                }
            }
        }
...
void set_default_log(char *level, char *pattern, char *filename, bool *enableFile, bool *enableConsole) {
    ...
    if(filename != NULL && strlen(filename) == 0) strcpy(filename, "translator_srv.log");
...
int get_deepl_api_from_config(const char *config_file, DeeplAPI *deepl_api_config) {
    FILE *file = fopen(config_file, "r");
    if (!file) {
        perror("fopen");
        return -1;
    ...
    bool found_deepl = false;
    bool found_deepl_api_key = false;
    bool found_deepl_base_url = false;
    bool parsing = true;
    bool error = false;
    ...
    while (parsing && !error) {
        ...
        if (token.type == YAML_KEY_TOKEN) {
            ...
            if (token.type == YAML_SCALAR_TOKEN) {
                if (!found_deepl && strcmp((char *)token.data.scalar.value, "deepl") == 0) {
                    found_deepl = true;
                }
                else if (found_deepl && !found_deepl_api_key && strcmp((char *)token.data.scalar.value, "auth_key") == 0) {
                    yaml_token_delete(&token);
                    ...
                    if (token.type == YAML_SCALAR_TOKEN) {
                        strcpy(deepl_api_config->api_key, (char*)token.data.scalar.value);
                        printf("api_key found: %s\n", deepl_api_config->api_key);
                        found_deepl_api_key = true;
                    }
                }
                else if (found_deepl && !found_deepl_base_url && strcmp((char *)token.data.scalar.value, "base_url") == 0) {
                    yaml_token_delete(&token);
                    ...
                    if (token.type == YAML_SCALAR_TOKEN) {
                        strcpy(deepl_api_config->base_url, (char*)token.data.scalar.value);
                        printf("base_url found: %s\n", deepl_api_config->base_url);
                        found_deepl_base_url = true;
                    }
                }

                parsing = !found_deepl_api_key || !found_deepl_base_url;
            }
        }
